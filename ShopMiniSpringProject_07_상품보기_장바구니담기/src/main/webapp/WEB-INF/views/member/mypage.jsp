...
$(function () {
	$("form").on("submit", function (e) {
		...
		/* if($("#username").val().length == 0 || $("#email1").val().length == 0 ||
			$("#email2").val().length == 0){
			alert("이름 및 이메일은 공백일 수 없습니다.")
			e.preventDefault();
		} */
	})
})
...
<br> 
이름:<input type="text" name="username" value="${login.username}" id="username"><br> 
...
전화번호:<select name="phone1">
  <option value="010" 
    <c:if test="${login.phone1 eq '010'}">selected</c:if>
  >010</option><!-- html태그 사이에서 jstl태그 사용  -->
  <option value="011"
    <c:if test="${login.phone1 eq '011'}">selected</c:if>
...
이메일:<input type="text" value="${login.email1 }" name="email1" id="email1">@
       ...
       <select  id="emailSelect">
        ...
        <c:if test="${login.email2 eq 'daum.net'}">selected</c:if>>
        daum.net</option>
        <option value="naver.com"
        <c:if test="${login.email2 eq 'naver.com'}">selected</c:if>>        
        naver.com</option>
       </select>
<br>
